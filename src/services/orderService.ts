...
import { OrderItem, OrderRecord } from '../models/order'
...
export const createOrder = (request: OrderRequest): OrderRecord => {
  const requestedDate = request.requestedAt ? new Date(request.requestedAt) : new Date()
  const executeDate = isMarketOpen(requestedDate) ? requestedDate : nextMarketOpen(requestedDate)
